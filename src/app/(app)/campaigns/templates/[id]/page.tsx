...
export default function EditTemplatePage() {
  ...
  useEffect(() => {
    async function load() {
      const { data } = await supabase
        ...
        .eq('id', templateId)
        .single()
      if (!data) { toast.error('Template not found.'); router.push('/campaigns/templates'); return }
      setName(data.name)
    ...
    return (
      <div className="px-8 py-10 max-w-5xl">
        <div className="bg-white/5 border border-white/10 rounded-2xl p-16 text-center">
          <p className="text-blue-300/60 text-sm">Loading template...</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="px-8 py-10 max-w-5xl">
      ...
      <h2 className="text-2xl font-bold text-white mb-1">Edit Template</h2>
      <p className="text-blue-300 text-sm mb-8">Update your email template.</p>
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden flex flex-col">
          ...
          <div className="p-5 flex-1 overflow-y-auto">
            <div className="mb-3">
              ...
              <p className="text-sm text-white font-medium">{previewText(subject)}</p>
            </div>
